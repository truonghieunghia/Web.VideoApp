...
public class PlaylistUpdates {
    ...
    public static JsonYoutubeList Result(String strurl) {
        String data = "";
        URL url;
        try {
            url = new URL(strurl);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            url = url + "&playlistId=PLC0emoCUH6bbJ7bRDSoLtuehLxq4Pxf0m&maxResults=20";

            JsonYoutubeList result = Result(url);

            while (result.nextPageToken!=null){
                String url_next = url+"&pageToken=" + result.nextPageToken;
                JsonYoutubeList result_next = Result(url_next);
                result.items.addAll(result_next.items);
                result.nextPageToken = result_next.nextPageToken;
            }
            ...
            String playlistId = "PLxq0gVtActrXZdZQl-32s5YJjImCwaFtL";//insertPlaylist();
            for(Channel obj:result.items){
                System.out.println("PlayListID: "+obj.id);
                System.out.println("VideoTitle: "+obj.snippet.title);
                System.out.println("VideoID: "+obj.snippet.resourceId.videoId);
                insertPlaylistItem(playlistId, obj.snippet.resourceId.videoId);
            }
